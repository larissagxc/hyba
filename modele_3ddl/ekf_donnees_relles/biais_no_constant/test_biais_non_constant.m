...
Q = diag([1,1,1,0.01,0.01,0.000001,0.00001]);
R = diag([1,1,0.01]);

% Initial guesses for the state vector and noise
ekf.State = [xm(1); ym(1); u(1); 0; psi0; 0; 0]; % Initial state estimate [x, y, u, v, psi]
ekf.ProcessNoise = Q; % Process noise covariance
ekf.MeasurementNoise = R; % Measurement noise covariance
...
ylabel('V_c (m/s)');
legend;
title('Reconstruction de V_c');

subplot(2,1,2);
% plot(t, psi, '--k', 'DisplayName', 'Measured v',LineWidth=2);
plot(t, xEstimated(7,:), 'b', 'DisplayName', 'Estimated \psi',LineWidth=2);
xlabel('Time (s)');
ylabel('\beta_c (rad)');
legend;
title('Reconstruction de l''angle d''attaque du courant');
