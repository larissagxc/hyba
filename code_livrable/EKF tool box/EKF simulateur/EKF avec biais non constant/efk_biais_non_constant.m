...
ekf.ProcessNoise = diag([1e-2, 1e-2, 1e-1, 1, 1e-1, 1e-6, 1e-6]); % Matrice Q
...
for k = 1:length(t)
    ...
    xTrue(1) = xTrue(1) + ((xTrue(3)+xTrue(6)) * cos(xTrue(5)) - (xTrue(4)+xTrue(7)) * sin(xTrue(5))) * dt;
...
Vc_est = xEstimated(6,:);
betac_est = xEstimated(7,:);

bu_est = -Vc*cos(betac_est-psi_est');
bv_est = -Vc*sin(betac_est-psi_est');
...
for i = 1:length(t)-1
    x_cal(i+1) = x_cal(i) + ((u_est(i)+bu_est(i))*cos(psi_est(i)) - (v_est(i)+bv_est(i))*sin(psi_est(i)))*dt;
    y_cal(i+1) = y_cal(i) + ((u_est(i)+bu_est(i))*sin(psi_est(i)) + (v_est(i)+bv_est(i))*cos(psi_est(i)))*dt;
end
...
for i = 1:10:length(t)
    ...
    set(h4, 'XData', y_cal(i), 'YData', x_cal(i), ...
        'UData', 50 * (u_est(i) + bu_est(i)) * sin(psi_est(i) + beta_est(i)), ...
        'VData', 50 * (u_est(i) + bu_est(i)) * cos(psi_est(i) + beta_est(i)));

    set(h5, 'XData', y_cal(i), 'YData', x_cal(i), ...
        'UData', 50 * (v_est(i) + bv_est(i)) * cos(psi_est(i) + beta_est(i)), ...
        'VData', -50 * (v_est(i) + bv_est(i)) * sin(psi_est(i) + beta_est(i)));
